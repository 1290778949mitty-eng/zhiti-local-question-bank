...
const knownCommands = new Set(('frac dfrac tfrac sqrt mathbb mathcal mathscr mathfrak mathrm mathit mathbf boldsymbol '
  + 'Rightarrow Leftarrow Leftrightarrow Longrightarrow Longleftarrow Longleftrightarrow rightarrow leftarrow '
  + 'therefore because angle triangle parallel perp times div cdot le leq ge geq ne neq approx '
  + 'alpha beta gamma theta pi infty in notin cup cap sin cos tan log ln lim begin end left right odot bigodot equiv circ square parallelogram text textrm' ).split(' '));
...
function repairCommandEscapes(text) {
  ...
  for (let i = 0; i < text.length;) {
    ...
    const repair = depth === 0 && slashes.length > 1 && knownCommands.has(name) && !isPath
      && (insideDollarMath(i) || (followsMathEnvironment && /^(?:Rightarrow|Leftarrow|Leftrightarrow|Longrightarrow|Longleftarrow|Longleftrightarrow)$/.test(name)));
