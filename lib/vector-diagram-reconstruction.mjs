import { isPhotographedDiagram } from "./image-processing-rules.mjs";
...
export function shouldAutoVectorizeDiagram(quality) {
  return Boolean(
    ...
    && (quality.score < 0.82 || isPhotographedDiagram(quality)),
  );
}
...
export function validateVectorDiagramPlan(plan) {
  ...
  if (plan.excludedAnnotations !== undefined && (!Array.isArray(plan.excludedAnnotations) || plan.excludedAnnotations.length > 50 || plan.excludedAnnotations.some((item) => typeof item !== "string" || item.length > 120))) return { ok: false, error: "过滤的学生标注记录不正确" };
  return { ok: true };
}
...
export function combineDiagramRasterFit(metrics, allowSourceAnnotations = false) {
  const contourScore = metrics.precision + metrics.recall ? 2 * metrics.precision * metrics.recall / (metrics.precision + metrics.recall) : 0;
  const strictScore = contourScore * .55 + metrics.projectionScore * .15 + metrics.boundsScore * .2 + metrics.toneScore * .1;
  // Student marks add source-only ink. Precision still verifies that every generated line
  // lands on the printed diagram without requiring the reconstruction to copy that ink.
  const annotationAwareScore = metrics.precision * .7 + metrics.projectionScore * .08 + metrics.boundsScore * .12 + metrics.toneScore * .1;
  return {
    contourScore,
    strictScore: Math.max(0, Math.min(1, strictScore)),
    annotationAwareScore: Math.max(0, Math.min(1, annotationAwareScore)),
    score: Math.max(0, Math.min(1, allowSourceAnnotations ? Math.max(strictScore, annotationAwareScore) : strictScore)),
  };
}
