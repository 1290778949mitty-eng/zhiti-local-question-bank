...
import { studioFullFiguresReady, studioFiguresReady } from './answer-studio-output';
...
export type StudioPipelineServices = {
  recognize(page:StudioPage, context:string):Promise<StudioRecord[]>;
...
export async function transcribeStudio(draft:StudioDraft, service:StudioPipelineServices, options:{drawings?:boolean;includeQuestionFigures?:boolean}={}) {
  ...
  for(const [index,page] of pages.entries()) {
    ...
    service.progress(`正在转录 ${index+1}/${pages.length} 页…`);
    const context=[...next.questions,...next.answers].slice(-18).map(q=>`${q.lesson}｜${q.section} ${q.number} ${q.stem.slice(0,150)} ${q.analysis.slice(-120)}`).join('\n');
    let records:StudioRecord[];
    try {
      records=(await service.recognize(page,context)).map(normalizeStudioTextFields);
    } catch(error) {
      const detail=error instanceof Error?error.message:'识别服务异常';
      throw new Error(`第 ${page.page} 页（${page.name}）转录失败：${detail}。已完成页面已保留，可点击“继续生成结果”重试。`,{cause:error});
    }
    if(page.role==='answer') {
    ...
    page.processed=true;
    // `pages` contains references to `next.pages`; checkpoint after every completed page.
    await save();
  }
  if(next.inputMode!=='answers') {
  ...
  for(const [index,q] of next.questions.entries()) {
    const answers=q.answerIds.map(id=>next.answers.find(a=>a.id===id)!);
    q.answerPlacements=answers.flatMap(a=>a.answerPlacements||[]);
    if(q.drawingsChecked&&q.drawingsRevision===STUDIO_DRAWINGS_REVISION&&(options.includeQuestionFigures?studioFullFiguresReady(q,next):studioFiguresReady(q,next)))continue;
    service.progress(`正在处理配图 ${index+1}/${next.questions.length} 题…`);
    ...
    if(evidence.length) {
      ...
      let result:StudioDrawingResult;
      try { result=await service.drawings(q,bases,evidence,context); if(studioDrawingMissing(q,bases,context,result))throw new Error('原件有配图，但未能确认是仅题目图或生成有效解答图'); }
      catch(error) { q.warnings.push(`配图处理失败：${error instanceof Error?error.message:'识别服务异常'}，已保留可用内容并继续导出`); q.drawingDisposition='uncertain'; drawingFailed=true; result={disposition:'uncertain',warnings:[],diagrams:[]}; }
