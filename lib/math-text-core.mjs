...
// Keep raw ranges so paragraph splitting cannot break an aligned/cases block.
function explicitRanges(text) {
  const ranges = [];
  let index = 0;
  while (index < text.length) {
    const delimiter = !escapedAt(text, index) && delimiters.find(item => text.startsWith(item.open, index));
    if (!delimiter) { index += 1; continue; }
    const start = index;
    const contentStart = index + delimiter.open.length;
    let end = contentStart;
    while (end < text.length) {
      if (text.startsWith(delimiter.close, end) && !escapedAt(text, end)
        && !(delimiter.close === "$" && (text[end + 1] === "$" || text[end - 1] === "$"))) break;
      end += 1;
    }
    if (end === text.length) {
      // An unmatched delimiter must remain visible, not become a fake equation.
      // Keep it as text; validation can report the raw marker separately.
      index = contentStart;
      continue;
    }
    ranges.push({ start, end: end + delimiter.close.length, value: text.slice(contentStart, end), display: delimiter.display });
