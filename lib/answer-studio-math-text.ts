...
function splitAutomatic(text: string): MathTextSegment[] {
  // OCR can return LaTeX without dollar delimiters. Never split a command's
  // braced argument at Chinese text or spaces and thereby corrupt the formula.
  if (/\\[A-Za-z]+/.test(text)) {
    return text.split(/(\r?\n)/).filter(Boolean).flatMap(line => /\\[A-Za-z]+/.test(line)
      ? [{kind:'math' as const,value:line,explicit:true}]
      : splitAutomatic(line));
  }
...
export function toLatexMath(source: string) {
  let latex: string = normalizeMathNotation(source).trim().replace(/\*\*/g, "^");
