...
const parallelContext=body.concurrent?"\n并发识别补充规则：所附上下文来自本批之前已确认页面，不保证是紧邻前一页。优先使用本页可见讲次、栏目与题号；页首明显是续解但归属不明时，仍逐字保留，continuation=true，并在warnings写明‘并发续页归属待确认’，不得套用不相关的旧题号。缺少栏目或讲次且无法从本页确定时也写入warnings，供随后顺序复核。":"";
    const result=await callRecognitionModel({apiKey,image:body.image,prompt:studioRecognitionPrompt(body.role,body.lesson,body.context||"")+extra+parallelContext,schema:studioRecognitionSchema,schemaName:"teacher_answer_transcription"});
