...
export function MathText({ text, className = "" }: { text: string; className?: string }) {
  const renderMathText = (value: string, keyPrefix: string) => splitMathText(value).map((segment, index) => {
    ...
    let html: string;
    try {
      html = katex.renderToString(toLatexMath(segment.value), { throwOnError: true, strict: "ignore", output: "htmlAndMathml" });
    } catch { return <span key={`${keyPrefix}-math-${index}`}>{segment.value}</span>; }
    return <span className="inline-math" key={`${keyPrefix}-math-${index}`} dangerouslySetInnerHTML={{ __html: html }} />;
  });
